...
public class Score {
	...
	protected boolean tieBreakGame = false;
	protected int tieBreakWinner = 0;
	...
	public boolean determineGameScore() {
		...
		// Tie-break game situation
		if (this.tieBreakGame == true && this.tieBreakWinner == 0) {

			this.player1ValueRes = player1Res;
			this.player2ValueRes = player2Res;

			if (player1Res > 6 || player2Res > 6) {

				this.scoreDifference = player1Res - player2Res;
				if (this.scoreDifference > 1) {
					this.setGameWinner(1);
					this.gameFinished = true;
					this.tieBreakWinner = 1;
					return true;
				} else if (this.scoreDifference < -1) {
					this.setGameWinner(2);
					this.gameFinished = true;
					this.tieBreakWinner = 2;
					return true;
				}
